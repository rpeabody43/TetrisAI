...
TEST(TestGenInput, BasicAssertions) {
    ...
    while (moves_played < 10 && !board.game_over()) {
        ...
        int8_t new_rot_delta =
            working_move.rotation - board.get_falling_piece_rot();
        int8_t offset =
            tetromino_data::get_piece_bounds(board.get_falling_piece(),
                                             board.get_falling_piece_rot())
                .left_bound;
        uint8_t target_col = Board::col(working_move.position - offset);
        uint8_t current_col =
            Board::col(board.get_falling_piece_anchor() - offset);
        int8_t new_col_delta = target_col - current_col;

        // Assert that the piece is closer to the "end" position than it was
        // before
        if (col_delta == 0) {
